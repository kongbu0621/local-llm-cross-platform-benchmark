...
def reject_aggregate_scores(path: Path, obj: Any) -> list[str]:
    ...
    for node in walk(obj):
        ...
        for key in sorted(bad):
            errors.append(
                f"forbidden aggregate score field {key}: {path.relative_to(ROOT)}; use Hard Gates + Qualification + Fitness"
            )
    return errors
...
def validate_qualification(path: Path, obj: dict[str, Any]) -> list[str]:
    ...
    if qualification in {"OPEN", "NOT_QUALIFIED"} and fitness is not None:
        errors.append(
            f"{path.relative_to(ROOT)}: qualification={qualification} requires fitness=null; unknown/fail must not be averaged into a score"
        )
...
def validate_topology_modes(path: Path, obj: dict[str, Any]) -> list[str]:
    ...
    for mode in obj.get("topology_modes", []):
        if mode.get("qualification") in {"OPEN", "NOT_QUALIFIED"} and mode.get("fitness") is not None:
            errors.append(
                f"{path.relative_to(ROOT)} topology={mode.get('mode')}: OPEN/NOT_QUALIFIED requires fitness=null"
            )
    return errors
...
def validate_recommendation(path: Path, obj: dict[str, Any]) -> list[str]:
    ...
    for option in options:
        if option.get("kind") in HARDWARE_KINDS and option.get("status") == "ACTION":
            ...
            if cf != "INSUFFICIENT":
                errors.append(
                    f"{path.relative_to(ROOT)} option={option.get('option_id')}: hardware ACTION requires counterfactual_no_hardware=INSUFFICIENT"
                )
            for key in ("trigger", "expected_gain", "does_not_solve", "residual_bottleneck", "new_failure_domains"):
                value = option.get(key)
                if value in (None, "", []):
                    errors.append(
                        f"{path.relative_to(ROOT)} option={option.get('option_id')}: hardware ACTION missing {key}"
                    )
    ...
    if isinstance(a, str) and isinstance(b, str):
        try:
            if datetime.fromisoformat(b) > datetime.fromisoformat(a):
                errors.append(f"{path.relative_to(ROOT)}: evidence_as_of cannot be later than evaluated_at")
        except ValueError:
            pass
    return errors
